...
### 2026-07-27 (later): The tool answers the questions an agent was reverse-engineering

Started as "why did an iPhone stand take 32 minutes" and the transcript answered it in a way no amount of reasoning would have. 39m39s, 128 assistant messages, 72 tool calls, and **383,791 output tokens of which about 18k appear as content**: roughly 95% was thinking, and the model spent ~5,300 tokens between consecutive tool calls. Turn count was not the problem and neither was CLI latency (`nurb check` is 3.1s, `verify` 4.6s, cold `import build123d` 2.7s of it). The Bash log said why it was thinking so hard: of 38 shell commands, **ten were reading nurb's own source in site-packages** (`polish.py`, `__init__.py`, `is_convex`, `concave_edges`, the `overhang` and `min_wall` rule bodies, `builder.py`, `registry.py`) and **twelve were throwaway Python that built the part and printed a face centre**. Twenty-two of thirty-eight commands were an agent reverse-engineering the tool and hand-rolling a measuring harness, which is the most think-expensive thing a model does.

Three commands came out of that, all of them the same lever: change what the tool hands back so the model needs to think less, since the harness's thinking budget belongs to whoever is driving and a generic tool cannot touch it. **`nurb api`** prints the vocabulary with signatures, derived by subtracting build123d's `__all__` from nurb's and reading summaries off the live functions, plus a shadowed section found by identity comparison so wrapping one more build123d name reports itself. **`nurb inspect`** measures a built part in the rules' own units: faces largest-first with area, droop, normal and centre, every concave edge, and each finding resolved to the face it fired on. Droop calls `checks._sample_normals` rather than reimplementing, because an inspector measuring overhang from horizontal while the rule measures from the build direction would leave the agent reconciling two numbers for one face, which is worse than not having it. **`chamfer` now shadows build123d's** and appends the doctrine's room rule on failure, because the kernel says "try a smaller length value(s)" and the doctrine is explicit that following it would never have found the real rule; taking it at its word is how a part ends up with a 0.4mm chamfer that lands and prints as a defect.

Four things this found that reading the code would not have:

- **A face's centre is frequently not on the face.** Resolving a finding to its face by containment missed the underside of any plate with a pocket in it, because that face is an annulus whose centroid sits in the hole. Matching the centre the rule recorded is what recovers it, with containment kept as the fallback for `min_wall`, whose `where` is a probe point rather than a centre. Found by a test failing, not by review.
- **Every part's bottom face reads as a 90 degree overhang and none of them is one.** Hiding the number would leave the agent unable to tell it from a real ceiling; printing it without the note invites a fix to a non-problem. It prints with `on the bed`, and the section header states the convention and the limit.
- **`fillet` must not get chamfer's advice, and wrapping it was a real bug.** build123d's fillet error already names `max_fillet()`, a genuine tool for finding the largest valid radius, and for a fillet "try a smaller value" is exactly the right fix. The doctrine's "shrinking is the wrong response" is chamfer-specific and applying it to both replaced good advice with advice that is only true half the time. The wrapper is chamfer-only; `nurb.fillet is build123d.fillet` is asserted so nobody re-adds it.
- **There is no one-off part, and a "skip the ceremony for one-offs" tier was written and then removed.** The reasoning was that the doctrine's bookkeeping is calibrated for a maintained catalog and a single part should not pay it. That is wrong about how these parts are actually used: a part gets printed, used in the real world, and revisited weeks or months later to adjust, at which point the session that built it is gone and `## Don't` is the only record of what was already tried and rejected. Deferring the card optimizes the first delivery and taxes the revisit, which is the moment the card exists for. The tier also failed its own evidence test, which is the part worth remembering: of the 38 shell commands in the measured session, **three** touched `nurb card` or `nurb verify`. Card ceremony was never the cost. It was invented as a problem because it looked like one, while the measured cost sat in twenty-two other commands. The paragraph now says the opposite, that the card gets written even for a single part, because the next session is the reader.

Also worth recording: the four shim copies (`src/nurb/skill.md`, `skills/nurb/SKILL.md`, `src/nurb/agents.md`, root `AGENTS.md`) are already pinned to each other by `test_the_skill_is_the_shim_with_a_trigger_on_top` and `test_the_shim_it_writes_is_the_one_the_package_ships`, and both fired during this work. The drift protection is complete and did not need extending.
