...
Five model and effort combinations across the three task classes, run 2026-08-02 and 2026-08-03 on subscription CLIs against nurb 0.9.0 and graded under the content revisions shown below. The spread is the corpus design working: the spec task (cable_clip) measures execution and tops out fast, the function task (bundle_holder) makes design judgment mechanical and separates the field, and the judgment task (leg_cup) measures measurement discipline the way nurb's doctrine defines it. The full rows and sanitized transcripts are committed under [submissions](submissions/); see [README.md](README.md) for how to run and submit a row.

The opus and sonnet bundle_holder rows are being re-run and are absent below. Their original trials ran under a 900s session cap that killed sessions mid-flight: opus twice ran it out polishing parts that were already perfect, and every sonnet trial was graded mid-thought, which is not a fair grade in either direction. The cap has since been raised to an hour and recorded in every row; trials that hit a cap are reported as censored, never as durations. This benchmark re-runs rather than re-scores when the run conditions were wrong.
...
- sonnet's withdrawn trials caught a real scorer bug, disclosed here: its curved cradle cross-sections made the retention search's iterated polygon intersections compound vertices without bound (1.2 million on one part), grinding past the one-minute grading cap, and two trials were wrongly zeroed for grader slowness before the deeper problem (grading interrupted sessions at all) forced the re-run. The scorer fix stands: sections are snapped to a micron grid, three orders below every scored tolerance; the fairness suite passes with exact reference scores and all nine previously committed bundle_holder parts re-grade identically.
