...
Size: 20.00 x 50.32 x 30.00 mm, 8087.7 mm3, 1 solid, 66 faces
...
Four faces under 1mm2, all of them 0.866mm2 corner triangles where three 1mm chamfers
meet at a convex corner. They are the front top corners of the two lips, two per post.
That is the only tiny face the doctrine allows outright, and a fifth means the polish
pass has started cutting something it should not.

Predicted from the exclusions before measuring: ten, the four above plus the two lip back
top corners per post and the slab's two front top corners. Measuring gave four, because
the sharp-corner exclusion above also reaches those six. The two counts differ by exactly
the edges that exclusion removes, which is the check that it does what it says.
...
tripped it. It now measures how far the face reaches, which separates them exactly: a
chamfer's reach is its size, 1.00, 2.00 and 3.00mm for bottom chamfers of those sizes,
against 4.29mm here. Width would not have: 3.25mm against 3.43mm.
