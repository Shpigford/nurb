...
**Finish with the polish pass.** Chamfered edges are what make a print feel designed rather than extruded, so the doctrine's last step (`polish`, 1mm on exposed edges) stays in the part through every edit; the template `nurb new` emits already ends with it. The one exposed edge that rounds instead of chamfering: the top rim of a closed perimeter wall gets `crown(wall)`, the bead that survives a roofline rising and falling, and asking for it any other way is how issue #55 spent 689 lines. Say so when you hand the part over, because the user can only ask for sharp edges if they know the chamfers are there on purpose. The viewer builds polished by default, and its polish button flips to the faster draft build.
